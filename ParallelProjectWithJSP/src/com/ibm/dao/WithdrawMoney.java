	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(dbCon!=null)
		{
			...
			try {
				...
				if(rs.next()) {
					if(rs.getInt("balance")<Integer.parseInt(request.getParameter("amnt"))) {
						response.getWriter().print("Insufficient Balance....try again");
						...
						pstmt.executeUpdate();
						response.getWriter().print("Withdrawn Successfully");
						request.getRequestDispatcher("mainMenu.jsp").include(request, response);
					}
				}	
